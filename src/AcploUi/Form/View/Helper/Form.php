    ...
    public function render(FormInterface $form, $isHorizontal = false, $labelColumns = 2)
    {
        ...
        foreach ($form as $element) {
            if ($element instanceof Button || $element instanceof Submit) {
                ...
                $formContent .= $this->view->plugin('acplo_form_collection')->render($element);
            } else {
                $formContent .= $this->view->plugin('acplo_form_row')->render($element, $this->isHorizontal, $this->labelColumns);
            }
        }

        if (count($buttons) > 0) {
            $formContent .= $this->view->plugin('acplo_form_row')->renderButtons($buttons, $this->isHorizontal, $this->labelColumns);
        }
